...
try:
    ...
    def trunc_normal_tf_(tensor, mean=0.0, std=1.0):
    ...
    def variance_scaling_(tensor, scale=1.0, mode="fan_in", distribution="truncated_normal"):
        ...
        if distribution == "normal":
            return nn.init.normal_(tensor, std=math.sqrt(variance))
    ...
    def lecun_normal_(tensor):
        ...
        std = math.sqrt(1.0 / max(1.0, float(fan_in)))
        return nn.init.trunc_normal_(tensor, std=std)
...
class SiglipVisionTransformer(nn.Module):
    ...
    def get_causal_mask(self, num_tokens_each_frame, num_layers, batch_size, num_heads, token_mask=None, cls_token=True, frame_independent_encoding=False, dtype=torch.float32):
        ...
        # If we are using flash-attention 2, then directly create a mask of size `(batch_size, seq_len)` where 0 stands
        # for the position of padding tokens and 1 for the position of non-padding tokens. We assume all padded tokens
        # will be put to the end of each sequence.
        ...
        if self.config.attn_type == 'bidirectional':
            mask = torch.ones(batch_size, N, N, device=device, dtype=dtype)
        elif self.config.attn_type == 'causal':
            mask = torch.tril(torch.ones(batch_size, N, N, device=device, dtype=dtype))
        elif self.config.attn_type == 'block_causal':
            ...
            for t in range(T):
                start = sum(num_tokens_each_frame[:t])
                end = sum(num_tokens_each_frame[:t + 1])
                mask[:, start:end, start:end] = 1
        else:
            raise NotImplementedError(f"Attention type {self.config.attn_type} is not supported.")

        if frame_independent_encoding and self.config.attn_type != 'block_causal':
            mask = torch.zeros(batch_size, N, N, device=device, dtype=dtype)
            for t in range(T):
                start = sum(num_tokens_each_frame[:t])
                end = sum(num_tokens_each_frame[:t + 1])
        ...
        if cls_token:
            ...
            if self.config.attn_type == 'causal':
                ...
                mask = torch.ones(batch_size, N + 1, N + 1, device=device, dtype=dtype)
            mask[:, 1:, 1:] = mask_
    ...
    def get_observation_mask(self, group_lengths, group_roles, batch_size, num_heads, token_mask=None, dtype=torch.float32):
        """
        Observation-level mask for:
          - history groups: causal among themselves
          - current tri-view groups: fully bidirectional
          - current head can read history
          - current wrists cannot directly read history
        """
        device = group_lengths.device
        lengths = [int(x) for x in group_lengths.tolist()]
        roles = [int(x) for x in group_roles.tolist()]
        ...
        mask = torch.zeros(batch_size, total_len, total_len, device=device, dtype=dtype)

        hist_ids = [i for i, role in enumerate(roles) if role == ROLE_HIST]
        head_ids = [i for i, role in enumerate(roles) if role == ROLE_HEAD]
        left_ids = [i for i, role in enumerate(roles) if role == ROLE_LEFT]
        right_ids = [i for i, role in enumerate(roles) if role == ROLE_RIGHT]
        current_ids = head_ids + left_ids + right_ids

        # History groups: block-causal.
        for hist_pos, group_idx in enumerate(hist_ids):
            qi0, qi1 = offsets[group_idx], offsets[group_idx + 1]
            for key_idx in hist_ids[: hist_pos + 1]:
                kj0, kj1 = offsets[key_idx], offsets[key_idx + 1]
                mask[:, qi0:qi1, kj0:kj1] = 1

        # Current tri-view: fully bidirectional.
        for group_idx in current_ids:
            qi0, qi1 = offsets[group_idx], offsets[group_idx + 1]
            for key_idx in current_ids:
                kj0, kj1 = offsets[key_idx], offsets[key_idx + 1]
                mask[:, qi0:qi1, kj0:kj1] = 1

        # Current head can read all history groups.
        for head_idx in head_ids:
            qi0, qi1 = offsets[head_idx], offsets[head_idx + 1]
            for key_idx in hist_ids:
                kj0, kj1 = offsets[key_idx], offsets[key_idx + 1]
                mask[:, qi0:qi1, kj0:kj1] = 1

        if token_mask is not None:
            token_mask = token_mask.unsqueeze(1).repeat(1, total_len, 1)
            mask = mask * (~token_mask).to(dtype)

        if self.config._attn_implementation in ("flex_attention", "sdpa", "eager"):
            mask = torch.where(mask == 1, 0, -torch.inf).to(dtype)
        elif self.config._attn_implementation == "flash_attention_2":
            raise NotImplementedError("Observation-level custom mask is not supported with flash_attention_2.")
        else:
            raise NotImplementedError(f"Attention implementation {self.config._attn_implementation} is not supported.")
    ...
    def forward(
        ...
        if gazing_info is not None and "group_roles" in gazing_info:
            encoder_attn_mask = self.get_observation_mask(
                gazing_info["num_gazing_each_frame"],
                gazing_info["group_roles"],
                B,
...
class SiglipVisionModel(SiglipPreTrainedModel):
    ...
    def __init__(self, config: SiglipVisionConfig):
        ...
        self.vision_model.embeddings.register_buffer(
            "position_ids",
            torch.arange(self.vision_model.embeddings.num_positions).expand((1, -1)),
            persistent=False,
        )
