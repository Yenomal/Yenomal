...
5. datasets需要ln包含进来，并且配置好ckpt，datasets的路径

## thinking

1. 复现路径：系统环境（后续可能考虑使用docker来配置，目前只能手动在主机下配置）+代码环境（git tag + manifest.sh）+python环境（uv，现在我打算通过uv完全监管python环境，包括import路径依赖+环境冲突，等到二进制出现冲突的时候使用conda配置二进制，或者使用docker容器，或者别的什么，这个就后续再看看）

2. baseline复现及其重要，主要是跑通datasets+ckpt，不一定要有结果，但是跑通很重要

3. 时常记住记录当前状况，避免出现问题就要重修，这也是为啥要跑baseline，**将一整个问题拆解成几个小问题**，比如datasets，环境，接口等等